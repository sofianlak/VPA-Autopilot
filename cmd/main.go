...
import (
	...
	"github.com/michelin/vpa-autopilot/internal/controller"
...
func init() {
	...
	utilruntime.Must(vpav1.AddToScheme(scheme))  // Needed to control VPA
	utilruntime.Must(appsv1.AddToScheme(scheme)) // Needed to check deployments
...
func main() {
	...
	flag.Var(&config.ExcludedNamespaces, "excluded-namespaces",
		"Comma separated list of namespaces excluded from the controller scope.")
	flag.StringVar(&config.ExcludedNamespaceLabelKey, "excluded-namespaces-key", "vpa-autopilot.michelin.com/vpa-autopilot-excluded",
		"The key used in the namespace labels to be ignored by the controller")
	flag.StringVar(&config.ExcludedNamespaceLabelValue, "excluded-namespaces-value", "true",
		"The value used in the namespace labels with ExcludedNamespaceLabelKey to be ignored by the controller")

	flag.BoolVar(&config.TargetLimits, "change-limits", false,
		"If set, the generated VPA will modify the limits of the targeted deployments")
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "5c84de1e.vpa-autopilot",
	...
	if err = (&controller.VerticalPodAutoscalerReconciler{
		...
		setupLog.Error(err, config.ControllerCreateError, "controller", "VerticalPodAutoscaler")
		os.Exit(1)
	}
	if err = (&controller.HorizontalPodAutoscalerReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, config.ControllerCreateError, "controller", "HorizontalPodAutoscaler")
		os.Exit(1)
	}
